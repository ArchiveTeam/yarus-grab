...
LUA_SHA1 = get_hash(os.path.join(CWD, 'yarus.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'post':
                wget_args.extend(['--warc-header', 'yarus-post: '+item_value])
                wget_args.append('https://yarus.ru/post/'+item_value)
            elif item_type == 'clip':
                wget_args.extend(['--warc-header', 'yarus-clip: '+item_value])
                wget_args.append('https://yarus.ru/clip/'+item_value)
            elif item_type == 'video':
                wget_args.extend(['--warc-header', 'yarus-video: '+item_value])
                wget_args.append('https://yarus.ru/video/'+item_value)
            elif item_type == 'user':
                wget_args.extend(['--warc-header', 'yarus-user: '+item_value])
                wget_args.append('https://yarus.ru/user/'+item_value)
            elif item_type == 'feed':
                wget_args.extend(['--warc-header', 'yarus-feed: '+item_value])
                wget_args.append('https://yarus.ru/feed/'+item_value)
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/3/3e/Yarus-icon.png" height="50px" title=""/>
        <h2>ЯRUS <span class="links"><a href="https://yarus.ru/">Website</a> &middot; <a href="http://tracker.archiveteam.org/yarus/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/ЯRUS">Wiki</a></span></h2>
        <p>Archiving ЯRUS.</p>
    '''
)
